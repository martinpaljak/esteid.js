    ...
    fields.connect = function(transmit) {
                ...
                .reduce(function(promise, item) {
                  return promise.then(function(result) {
                    ...
                    return apdu
                      .check(
                        transmit(
                          Buffer.from([0x00, 0xb2, item + 1, 0x04, 0x00])
                        )
                      )
                  ...
                  return promise.then(function(result) {
                    ...
                    return apdu
                      ...
                      .then(function(result) {
                        ...
                        personaldata[item] =
                          recordvalue.length === 1 && recordvalue[0] === 0x00
                            ? ""
                            : recordvalue.toString("latin1").trim();
                      })
                      .catch(function(err) {
      ...
      function verify(pin, value) {
        ...
        return value.then(function(pv) {
          ...
          return transmit(Buffer.concat([cmd, pinvalue])).then(function(
            response
          ) {
            ...
            if (sw === 0x9000) {
              ...
              throw new Error(
                "Incorrect PIN, tries left: " + (sw & 0xf).toString(16)
              );
            } else {
      ...
      function getCertificate(type) {
        ...
        return apdu
          ...
          .then(function() {
            // read aace or ddce
            return apdu.check(
              transmit("00a4020c02" + (type === AUTH ? "aace" : "ddce"))
            );
          })
          .then(function() {
            // NB! requires sequential execution
            ...
            function readfrom(offset, total) {
              ...
              return apdu.check(transmit(cmd), 0x6282).then(
                function(response) {
                  ...
                  if (chunk.length === 0 || cert.length >= total) {
                    ...
                    return readfrom(offset + chunk.length, total);
                  }
                },
                function(reason) {
      ...
      function change(pin, oldvalue, newvalue) {
        ...
        return oldvalue
          ...
          .then(function(pv) {
            ...
            return transmit(Buffer.concat([cmd, oldpin, newpin])).then(function(
              response
            ) {
              var sw = apdu.sw(response);
              if ((sw & 0x6300) === 0x6300) {
