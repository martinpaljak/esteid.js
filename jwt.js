...
var base64urldecode = function(t) {
  return Buffer.from(
    (t + "===".slice((t.length + 3) % 4)).replace(/-/g, "+").replace(/_/g, "/"),
    "base64"
  ).toString();
};
...
function token(c, nonce, origin) {
  ...
  r.header = JSON.stringify({
    alg: "RS256",
    typ: "JWT",
    x5c: [c.toString("base64")]
  });
  // TODO: get rid of this dependency
  const crt = Certificate.fromPEM(pem(c));
  ...
  r.dtbs = function() {
    return Buffer.from(
      base64urlencode(this.header) + "." + base64urlencode(this.payload)
    );
  };
